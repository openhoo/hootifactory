...
- **Formats:** npm, Docker, OCI, PyPI, Helm, Go, Cargo, NuGet, APT (9 total)
  — all verified end-to-end against Dockerized real clients (APT serves unsigned
  `Release` files consumed with `[trusted=yes]`).
...
run npm, Docker, ORAS, pip/twine, Helm, Go, Cargo, dotnet and apt-get through pinned

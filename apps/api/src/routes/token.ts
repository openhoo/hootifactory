...
import { grantDockerScope } from "./token-scopes";
...
async function handleToken(c: Context<AppEnv>): Promise<Response> {
  ...
  if (!parsedQuery.ok) return c.json(parsedQuery.body, parsedQuery.status);
  const { service, scopes } = parsedQuery.data;
  setActiveSpanAttributes({
    "registry.token.scope_count": scopes.length,
    "registry.token.service": service ?? "",
    "auth.principal.kind": principal.kind,
  });

  if ((scopes.length > 0 || service !== undefined) && service !== REGISTRY_TOKEN_SERVICE) {

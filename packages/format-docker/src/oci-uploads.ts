...
async function loadSession(image: string, uuid: string, ctx: RepoContext) {
...
async function loadSessionForUpdateTx(image: string, uuid: string, ctx: RepoContext, tx: Tx) {
  const parsedUuid = parseUploadUuid(uuid);
...
async function loadOpenSession(image: string, uuid: string, ctx: RepoContext) {
...
async function loadOpenSessionForUpdateTx(image: string, uuid: string, ctx: RepoContext, tx: Tx) {
...
function openSessionWhere(ctx: RepoContext, image: string, uuid: string) {
  return and(
    eq(uploadSessions.id, uuid),
    eq(uploadSessions.repositoryId, ctx.repo.id),
    eq(uploadSessions.scope, image),

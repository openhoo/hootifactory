...
export class GenericAdapter implements RegistryPlugin {
  ...
  private async download(
    ...
    return serveRegistryBlob(ctx, {
      ...
      // Checksum sidecars. We deliberately do not set `content-length`: the blob
      // is served as a raw ReadableStream, for which Bun strips an explicit
      // content-length and sends the body chunked, so the header would be a no-op.
      extraHeaders: {
        etag: `"${meta.sha256}"`,
  ...
  private async remove(pathRaw: string, ctx: RegistryRequestContext): Promise<Response> {
    ...
    const row = await ctx.data.versions.findLive(pkg, GENERIC_VERSION);
    const meta = parseGenericVersionMeta(row?.metadata);
    if (!meta || !row) return new Response("Not Found", { status: 404 });

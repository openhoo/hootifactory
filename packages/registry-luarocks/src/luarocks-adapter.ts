...
class LuarocksAdapterState {
  ...
  async publish(filename: string, req: Request, ctx: RegistryRequestContext): Promise<Response> {
    const parsed = parseArtifactFilename(filename);
   ...
   */
  async apiCheckRockspec(req: Request, ctx: RegistryRequestContext): Promise<Response> {
   ...
   * `module_url`.
   */
  async apiUpload(req: Request, ctx: RegistryRequestContext): Promise<Response> {
...
const luarocksDefinition = registryAdapter("luarocks")
  .stateClass(LuarocksAdapterState)
  .module((module) =>
    module
      .displayName("LuaRocks")
      .mount("luarocks")
      // Only `virtualizable` is honest: there is no `proxyIngest` or upstream
      // mirroring implementation for LuaRocks.
  ...
  .routes((route) => [
    ...
    ...VERSIONED_MANIFESTS.map((name) =>
      route.get(`/${name}`, "manifest").calls((state, { req, ctx }) => state.manifest(req, ctx)),
    ),
    // LuaRocks upload API (`luarocks upload --api-key=<key>`). The literal
    // `/api/1/...` routes precede the `:filename` catch-all so they win.
    route
      .get("/api/1/:apikey/check_rockspec", "apiCheckRockspec")
      .calls((state, { req, ctx }) => state.apiCheckRockspec(req, ctx)),
    route
      .post("/api/1/:apikey/upload", "apiUpload")
      .calls((state, { req, ctx }) => state.apiUpload(req, ctx)),
    route
      .post("/api/1/:apikey/upload_rock/:versionId", "apiUploadRock")
      .calls((state, { params, req, ctx }) => state.apiUploadRock(params.versionId, req, ctx)),
    route
      .get("/:filename", "download")
      .calls((state, { params, req, ctx }) => state.download(params.filename, req, ctx)),
    route
      .put("/:filename", "publish")

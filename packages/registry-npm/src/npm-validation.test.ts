...
describe("npm validation helpers", () => {
  ...
  test("derives exact versions from tarball filenames", () => {
    expect(versionFromTarballFilename("pkg", "pkg-1.2.3.tgz")).toBe("1.2.3");
    expect(versionFromTarballFilename("@scope/pkg", "pkg-1.2.3-beta.1.tgz")).toBe("1.2.3-beta.1");
    expect(versionFromTarballFilename("pkg-with-dash", "pkg-with-dash-1.0.0+build.5.tgz")).toBe(
      "1.0.0+build.5",
    );
    expect(versionFromTarballFilename("@scope/pkg", "other-1.2.3.tgz")).toBeNull();
    expect(versionFromTarballFilename("pkg", "pkg-latest.tgz")).toBeNull();
  });

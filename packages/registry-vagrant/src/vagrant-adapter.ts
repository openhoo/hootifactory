 ...
 * `GET /:user/:box/:version/:provider`. Publish is a hootifactory extension:
 * `PUT /:user/:box/:version/:provider` uploads a `.box` for a (version, provider)
 * pair, and the metadata is regenerated from the live versions on read.
 */
export class VagrantAdapter implements RegistryPlugin {
  ...
  private async metadata(
    ...
    for (const row of rows) {
      ...
      versions.push(
        buildVagrantMetadataVersion(row.version, meta, (provider) =>
          this.downloadUrl(ctx, user, box, row.version, provider),
        ),
      );
    }
